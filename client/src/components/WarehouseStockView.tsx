...
export const WarehouseStockView: React.FC<WarehouseStockViewProps> = ({
  ...
  const [itemToDelete, setItemToDelete] = useState<WarehouseStockItem | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.deleteCatalogItem(itemToDelete.hardware_id);
      setFeedback({
        type: 'success',
        message: `Hardware item "${itemToDelete.hardware_name}" (${itemToDelete.sku}) was successfully deleted.`
      });
      ...
      setFeedback({
        type: 'error',
        message: err.message || 'Failed to delete hardware item'
      });
    } finally {
      setIsDeleting(false);
    }
                    ...
                    >
                      ...
                      <td className="py-3 px-3.5 text-right">
                        <div className="flex items-center justify-end space-x-1.5">
                          {/* Customize Par Level or Adjust Stock & Delete (ADMIN ONLY) */}
                          {currentRole === 'ADMIN' && (
                            <>
                              <button
                              ...
                              <button
                                ...
                                className="px-2 py-1 text-rose-600 hover:text-white bg-rose-50 hover:bg-rose-600 border border-rose-200 rounded font-semibold text-[11px] transition flex items-center space-x-1 cursor-pointer"
                                title="Delete hardware item from catalog"
                              >
          ...
          <div className={`bg-white rounded-2xl shadow-2xl ${modalMode === 'CREATE_CUSTOM' ? 'max-w-2xl' : 'max-w-lg'} w-full overflow-hidden border border-slate-200 flex flex-col max-h-[92vh]`}>
            ...
            <form onSubmit={handleSaveCustomization} className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                {modalMode === 'EDIT_PAR' && selectedStockForEdit && currentRole === 'ADMIN' ? (
                  <button
                    type="button"
                    onClick={() => {
                      const item = selectedStockForEdit;
                      setShowCustomizeModal(false);
                      setItemToDelete(item);
                    }}
                    className="px-3 py-2 border border-rose-200 text-rose-600 hover:bg-rose-50 rounded-lg font-medium transition flex items-center space-x-1.5 cursor-pointer text-xs"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                ...
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={() => setShowCustomizeModal(false)}
                    className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-100 font-medium transition cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold shadow-sm transition cursor-pointer"
                  >
                    {modalMode === 'CREATE_CUSTOM'
                      ? warehouseAssignmentMode === 'ALL_ACTIVE'
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200 animate-fadeIn">
            <div className="p-6">
              ...
              <p className="text-xs text-slate-600 text-center mb-4">
                ...
                <strong className="text-slate-900">{itemToDelete.hardware_name}</strong> ({itemToDelete.sku})?
              </p>

              <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl text-[11px] text-rose-900 space-y-1 mb-5">
                <p className="font-semibold text-rose-800">⚠️ This action will permanently remove:</p>
                <ul className="list-disc list-inside space-y-0.5 text-rose-700">
                  <li>Item specification and catalog listing</li>
                  <li>Stock records across all warehouse hubs</li>
                  <li>Related inventory ledgers and stock requests</li>
                </ul>
              </div>
              ...
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-700 hover:bg-slate-100 rounded-xl font-medium transition text-xs cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-semibold shadow-sm transition text-xs flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
                >
                  {isDeleting ? (
                    <span>Deleting...</span>
                  ) : (
