  ...
  try {
    ...
    if (new_category_name && new_category_name.trim()) {
      ...
      if (existingCat) {
        ...
        const catInsert = db.prepare('INSERT INTO hardware_categories (name, description, icon) VALUES (?, ?, ?)');
    ...
    await db.transaction(async () => {
      ...
      if (apply_to_all_warehouses || initial_warehouse_id === 0 || initial_warehouse_id === 'ALL') {
        const activeWarehouses = await db.prepare('SELECT id, code, name FROM warehouses WHERE is_active = 1').all();
...
router.post('/requests', async (req, res) => {
  try {
    ...
    // Generate Request No
    const count = (await db.prepare('SELECT COUNT(*) as count FROM hardware_requests').get())?.count || 0;
...
router.post('/requests/:id/dispatch', async (req, res) => {
  try {
    ...
    const dispatchTx = db.transaction(async () => {
...
router.post('/replenishments', async (req, res) => {
  try {
    ...
    const count = (await db.prepare('SELECT COUNT(*) as count FROM replenishments').get())?.count || 0;
...
router.post('/replenishments/:id/warehouse-accept', async (req, res) => {
  try {
    ...
    const acceptTx = db.transaction(async () => {
      for (const it of items) {
...
router.post('/replenishments/:id/branch-confirm', async (req, res) => {
  try {
    ...
    const confirmTx = db.transaction(async () => {
      await db.prepare(`
        UPDATE replenishments
        SET 
